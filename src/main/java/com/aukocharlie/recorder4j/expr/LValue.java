 ...
 ***/
    ...
    static LValue makeBoolean(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, token.image.charAt(0) == 't');
    }

    static LValue makeCharacter(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, token.image.charAt(1));
    }

    static LValue makeFloat(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Float.valueOf(token.image).floatValue());
    }

    static LValue makeDouble(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Double.valueOf(token.image).doubleValue());
    }

    static LValue makeInteger(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Integer.parseInt(token.image));
    }

    static LValue makeShort(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Short.parseShort(token.image));
    }

    static LValue makeLong(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Long.parseLong(token.image));
    }

    static LValue makeByte(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token) {
        return make(vm, Byte.parseByte(token.image));
    }

    static LValue makeString(VirtualMachine vm,
                             com.aukocharlie.recorder4j.expr.Token token) throws ParseException {
    ...
    static LValue booleanOperation(VirtualMachine vm, com.aukocharlie.recorder4j.expr.Token token,
                                   LValue rightL,
